...
# SQL for owned cards ranked by current market value (nonfoil or foil as owned).
TOP_OWNED_CARDS_QUERY = """
SELECT
    c.set_code,
    c.collector_number,
    c.name,
...
JOIN purchases p
    ON p.set_code = c.set_code
    AND p.collector_number = c.collector_number
ORDER BY current_value DESC, c.set_code, CAST(c.collector_number AS INTEGER)
"""
