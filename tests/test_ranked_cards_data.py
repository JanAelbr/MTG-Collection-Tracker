...
class RankedCardsDataTests(unittest.TestCase):
    def test_owned_without_price_keeps_owned_and_expands_sibling_finishes(self):
        ...
        owned = expanded[expanded["finish"] == 0].iloc[0]
        self.assertEqual(owned["purchase_value"], 1.5)
        self.assertTrue(pd.isna(owned["current_value"]))
        foil = expanded[expanded["finish"] == 1].iloc[0]
        self.assertTrue(pd.isna(foil["purchase_value"]))
    ...
    def test_unowned_with_finish_flags_but_no_prices_expands_all_flags(self):
        cards_df = pd.DataFrame([
            {
                "set_code": "HOU",
                "collector_number": "73",
        ...
        self.assertEqual(finishes, [0, 1])
        self.assertTrue(expanded["purchase_value"].isna().all())

    def test_unowned_with_prices_expands_catalog_finishes(self):
        cards_df = pd.DataFrame([
            {
                "set_code": "HOU",
                "collector_number": "73",
    ...
    def test_etched_finish_expands_with_siblings(self):
        cards_df = pd.DataFrame([
            {
                "set_code": "MH2",
