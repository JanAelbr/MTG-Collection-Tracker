function getRisersForPage(data, compareDate, filters) {
    return applyPageSizeLimit(
        applyRankedCardFilters(data.cards, filters, data)
            ...
            .sort((left, right) => {
                if (right.price_change !== left.price_change) {
                    return right.price_change - left.price_change;
                }
...
function renderTopImages(risers) {
    const container = ensureTopImagesContainer();
    if (!container) {
        return;
    }

    const figures = risers.slice(0, 5).map((card, index) => {
        if (!card.image_uri) {
            return '';
        }
        return (
...
function renderRisersTable(risers, compareLabel, compareDate, ownedFilter) {
    ...
    tbody.innerHTML = risers.map((card, index) => (
        '<tr>' +
        `<td>${index + 1}</td>` +
...
function renderRisersForCompareDate(data, compareDate, compareLabel, filters) {
    const risers = getRisersForPage(data, compareDate, filters);
    renderRisersTable(risers, compareLabel, compareDate, filters.ownedFilter);
    renderTopImages(risers);
}

function initRisersReport() {
    const data = window.RISERS_DATA;
    if (!data) {
        return;
    }
    ...
    function renderCurrentSet() {
        renderRisersForCompareDate(data, compareDate, compareLabel, filters);
    }
...
$(document).ready(initRisersReport);
