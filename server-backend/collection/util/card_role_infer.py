...
_MLD = re.compile(r"\b(destroy|exile) all lands\b", re.I)
_GRAVEYARD_HATE = re.compile(
    r"\bexile .{0,80}?from .{0,40}?graveyard",
    re.I,
)
_RECURSION = re.compile(
    r"\breturn .{0,80}?from (your|a|target) graveyard to (your hand|the battlefield)\b",
    re.I,
)
...
_PROTECTION_KEYWORD = re.compile(
    ...
    r"can't be the target|prevent all damage)\b",
    re.I,
)
...
def _is_interaction(oracle: str) -> bool:
    return _is_removal(oracle) or bool(_COUNTER_SPELL.search(oracle))
...
def infer_card_roles(card: dict) -> list[str]:
    ...
    if oracle:
        if _is_ramp(oracle, type_line):
            roles.append("ramp")
        if _is_tutor(oracle):
            roles.append("tutor")
        if _is_draw(oracle):
            roles.append("draw")
        if _is_removal(oracle):
            roles.append("removal")
        if _is_protection(oracle):
            roles.append("protection")
        if _is_interaction(oracle):
            roles.append("interaction")
        if _is_recursion(oracle):
            roles.append("recursion")
        if _is_graveyard_hate(oracle):
            roles.append("graveyard_hate")
        if _is_extra_turn(oracle):
            roles.append("extra_turn")
        if _is_mld(oracle):
            roles.append("mass_land_destruction")
    return roles
