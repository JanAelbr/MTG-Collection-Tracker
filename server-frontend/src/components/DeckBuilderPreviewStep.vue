...
const themeChips = computed(() => {
  const profile = props.proposal?.theme?.profile;
  if (!profile) {
    return [];
  }
  const chips = [];
  for (const row of profile.primary || []) {
    chips.push({ id: row.id, label: String(row.id).replace(/_/g, " ").replace("tribal:", "") });
  }
  for (const row of profile.tribal || []) {
    const label = String(row.id);
    if (!chips.some((chip) => chip.id === label || chip.id === `tribal:${label}`)) {
      chips.push({ id: label, label });
    }
  }
  return chips.slice(0, 8);
});

const themeSummary = computed(() => props.proposal?.theme || null);
const diff = computed(() => props.proposal?.diff || null);
  ...
  <section class="deck-builder-step deck-builder-preview-step">
    ...
    <template v-else-if="proposal">
      <div class="deck-builder-preview-summary">
        ...
        <span v-if="themeSummary" class="deck-builder-stat-chip">
          Theme {{ Math.round((themeSummary.density || 0) * 100) }}%
        </span>
        <span v-if="proposal.stats?.utilityLandCount" class="deck-builder-stat-chip">
          {{ proposal.stats.utilityLandCount }} utility lands
        </span>
      </div>
      <div v-if="themeChips.length" class="deck-builder-theme-chips" aria-label="Commander themes">
        <span
          v-for="chip in themeChips"
          :key="chip.id"
          class="deck-builder-theme-chip"
        >
          {{ chip.label }}
        </span>
        <span
          v-if="themeSummary?.missingNeeds?.length"
          class="deck-builder-theme-chip deck-builder-theme-chip--gap"
        >
          Needs: {{ themeSummary.missingNeeds.slice(0, 3).join(", ") }}
        </span>
      </div>
      <div
        v-if="diff && (diff.add?.length || diff.cut?.length)"
        class="deck-builder-diff-panel"
      >
        <h4>Changes</h4>
        <p>
          Keep {{ diff.keep?.length || 0 }}
          · Add {{ diff.add?.length || 0 }}
          · Cut {{ diff.cut?.length || 0 }}
        </p>
        <div class="deck-builder-diff-columns">
          <div v-if="diff.add?.length">
            <h5>Add</h5>
            <ul>
              <li v-for="row in diff.add.slice(0, 12)" :key="`add-${row.name}`">
                {{ row.name }}
                <span class="deck-builder-diff-reason">{{ row.reason }}</span>
              </li>
            </ul>
          </div>
          <div v-if="diff.cut?.length">
            <h5>Cut</h5>
            <ul>
              <li v-for="row in diff.cut.slice(0, 12)" :key="`cut-${row.name}`">
                {{ row.name }}
                <span class="deck-builder-diff-reason">{{ row.reason }}</span>
              </li>
            </ul>
            ...
            <ul class="deck-builder-card-rows">
              ...
              >
                <span class="deck-builder-card-name">
                  ...
                  <span
                    v-if="card.themeHits?.length"
                    class="deck-builder-card-theme"
                    :title="card.themeHits.join(', ')"
                  >
                    {{ card.themeHits.slice(0, 2).join(", ") }}
                  </span>
                </span>
                <span
